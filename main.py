		...
		if i is server:
			...
			if game['clients'][i]['choice'] == 1:
				if not len(inpectors):
					...
					if game['lake'] > -8:
						game['lake'] -= 1
					text.insert(END, '\t{} Получил:{}$ ({}), озеро:{}\n'.format(game['clients'][i]['addr'], lake_money[game['lake'] + 8][0], game['clients'][i]['money'], game['lake']))
					text.see(END)
				else:
					game['clients'][i]['money'] -= 20
					text.insert(END, '\t{} Оштрафован на 20$ игроками:{} ({})\n'.format(game['clients'][i]['addr'], ' '.join([i['addr'] for i in inpectors]),game['lake']))
					text.see(END)
			elif game['clients'][i]['choice'] == 2:
				game['clients'][i]['money'] += lake_money[game['lake'] + 8][1]
				if game['lake'] < 6:
					game['lake'] += 1
				text.insert(END, '\t{} Получил:{}$ ({}), озеро:{}\n'.format(game['clients'][i]['addr'], lake_money[game['lake'] + 8][1], game['clients'][i]['money'], game['lake']))
				if len(presentors):
					text.insert(END, '\t{} примирован на 10$ игроками:{} ({})\n'.format(game['clients'][i]['addr'], ' '.join([i['addr'] for i in presentors]), game['clients'][i]['money']))
				text.see(END)
			elif game['clients'][i]['choice'] == 3:
				game['clients'][i]['money'] += 8
				text.insert(END, '\t{} Получил:8$ ({})\n'.format(game['clients'][i]['addr'], game['clients'][i]['money']))
				text.see(END)
